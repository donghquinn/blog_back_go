...
func PostRouter(server *http.ServeMux) {
	...
	server.HandleFunc("POST /post/list/pinned", controllers.GetPinnedPostController)
	...
	server.HandleFunc("POST /post/category/list", controllers.GetCategoryController)
}